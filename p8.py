...
path = process_csv("small_mapping.csv")
path
...
nm0503567_value = small_mapping["nm0503567"]
nm0503567_value
...
nm_values = []
for ID in small_mapping:
    if ID.startswith("nm"):
        nm_values.append(small_mapping[ID])
nm_values 
...
name = ''
for ID in small_mapping:
    name = small_mapping[ID]
    if ID.startswith('nm') and name[-5:] == "Jones":
        nm_jones.append(ID)
...
last_movie_directors_small_movies = small_movies_data[len(small_movies_data) -1]['directors']
...
movies_20220_20230 = movies[20220:20230]
movies_20220_20230
...
kubrick_films = find_specific_movies(movies, "Stanley Kubrick")
kubrick_films
...
wisc_list = copy.copy(movies)
wisconsin_movies = find_specific_movies(wisc_list, "Wisconsin")
wisconsin_movies
...
def bucketize_by_genre(movies):
    ...
    for movie in movies:
        for genres in movie['genres']:
            if genres not in new_dict:
                new_dict[genres] = []
            new_dict[genres].append(movie)
...
for key in genre_dict["Film-Noir"]:
    if key["rating"] > 8.0:
        film_noir_movies_above_8.append(key["title"])
film_noir_movies_above_8
...
number_movies_total = 0 

for genre in genre_dict:
    number_movies = 0 
    for film in genre_dict[genre]:
        for name in film["cast"]:
            if name == "Blake Lively":
                number_movies += 1
    if number_movies >= number_movies_total or blake_lively_genre == None:
        blake_lively_genre = genre
        number_movies_total = number_movies
...
for movie in genre_dict['Comedy']:
    if movie['rating'] >= max_comedy_rating or max_comedy_rating == 0:
        max_comedy_rating = movie['rating']

for movie in genre_dict['Comedy']:
    if movie['rating'] == max_comedy_rating:
        for director in movie['directors']:
            max_comedy_rating_directors.append(director)
